...
function generateServerCode(
  ...
  // Calculate relative path from output directory to config file using path.relative
  const configImportPath = getRelativeImportPath(outDir, configPath, projectRoot);
...
export async function writeServer(
  ...
  const code = generateServerCode(configPath, outDir, projectRoot, port);
...
export async function writeVersionedServer(
  ...
  const code = generateVersionedServerCode(configPath, versionKeys, outDir, projectRoot, port);
  await fs.writeFile(serverPath, code, "utf-8");
}
